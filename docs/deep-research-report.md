...
本方案不声称发明新的检索算法，也不声称是第一个 DeepSeek Harness + RAG 集成。公开生态已经提供了运行时、长期记忆、分层上下文、知识库插件、RAG 后端和模块化检索理论。本论文的研究问题是：如何把这些能力组织成一个面向多品牌企业的、可隔离、可验证、可编排、可恢复的控制平面。

相关项目及其边界详见 [`docs/related-work.md`](related-work.md)。这里采用一个更可验证的主张：

> 对多知识域企业任务，将全局插件路由、局部检索、证据验证和确定性工作流分开，应该比单一无边界 RAG 或不受约束的 Agent Loop 更少发生知识域串线，并且更容易定位和恢复失败。

因此，插件路由不应永远强制 Top-1：高置信度走单插件；中等置信度走受限 Top-2/Top-3 并比较证据；低置信度或证据冲突时先检索域摘要、重写查询或进入 `HOLD`。该假设必须通过 [`evaluations/comparative-evaluation.md`](../evaluations/comparative-evaluation.md) 的基线、负例和回执进行验证，不能只凭架构图宣称“更优”。
